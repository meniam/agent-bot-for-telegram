...
def _make_streamer() -> DraftStreamer:
    """Build a DraftStreamer wrapping a mocked bot."""
    bot = MagicMock()
...
def test_build_draft_html_truncates_thinking() -> None:
    """Thinking text is truncated to the given limit."""
...
async def test_stream_returns_text_only() -> None:
    """Streaming returns only the concatenated text chunks."""
...
async def test_stream_calls_send_draft() -> None:
    """Streaming calls the bot's send_rich_message_draft."""
    from collections.abc import AsyncGenerator
...
async def test_stream_empty_chunks() -> None:
    """Streaming an empty chunk source returns an empty string."""
    from collections.abc import AsyncGenerator
