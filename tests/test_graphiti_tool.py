...
async def test_forces_group_id_overriding_model() -> None:
    ...
    proxy = _FakeProxy()
    handle = make_graphiti_handler(CHAT, proxy, _spec("add_memory", gid=True, gids=False))
...
async def test_forces_group_ids_overriding_model() -> None:
    ...
    proxy = _FakeProxy()
    handle = make_graphiti_handler(CHAT, proxy, _spec("search_nodes", gid=False, gids=True))
...
async def test_leaves_ungrouped_tool_untouched() -> None:
    ...
    proxy = _FakeProxy()
    handle = make_graphiti_handler(CHAT, proxy, _spec("delete_entity_edge", gid=False, gids=False))
...
async def test_handler_wraps_upstream_errors() -> None:
    ...
    handle = make_graphiti_handler(CHAT, _Boom(), _spec("add_memory", gid=True, gids=False))
