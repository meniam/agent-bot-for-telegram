...
что-то сломалось. Правила — общий `changelog.md`. Проект не
...
- Исправлено: пользовательские команды с именами `sess`, `task` и `tasks`
  больше не загружаются из `commands_dir`: эти имена заняты встроенными
  командами, как и остальные из списка в `COMMANDS.md`.
